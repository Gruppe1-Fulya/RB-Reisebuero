...
public class FullDB{
    ...
    protected static void insertKunde(Kunde kunde) {
        ...
        try {
            ...
            preparedStatement.setString(2, String.valueOf(kunde.getKontostand()));
            preparedStatement.execute();
    ...
    protected static ObservableList<Kunde> getKundeDB() {
        ...
        try {
            oblist.clear();
            query = "SELECT * FROM kunde INNER JOIN person ON person.id = kunde.kundeid";
            preparedStatement = connection.prepareStatement(query);
            resultSet = preparedStatement.executeQuery();

            while (resultSet.next()) {
                Kunde kunde = new Kunde(resultSet.getInt("kundeid"), resultSet.getString("name"),
                        resultSet.getString("nachname"), resultSet.getInt("telefonnummer"),
    ...
    protected static ResultSet getKunde() {
        try {
            query = "SELECT * FROM person INNER JOIN kunde WHERE person.id = kunde.kundeid";
            preparedStatement = connection.prepareStatement(query);
            resultSet = preparedStatement.executeQuery();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    protected static void makePaid(Zahlung zahlung) {
        try {
            ...
            String query2 = "UPDATE kunde SET kontostand = kontostand + " + zahlung.getZahlungsbetrag() + " WHERE kundeid = '" + zahlung.getKundeid() + "'";
            preparedStatement = connection.prepareStatement(query);
            preparedStatement.execute();
            preparedStatement = connection.prepareStatement(query2);
            preparedStatement.execute();
